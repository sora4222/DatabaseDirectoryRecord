...
public class DatabaseQuery {
  ...
  private static final String allComputerFileCheck =
    "SELECT AbsoluteFilePath, FileHash FROM `directory_records` " +
  ...
  private static final String checkSingleFileQuery = "SELECT AbsoluteFilePath FROM `directory_records` " +
    "INNER JOIN file_paths ON directory_records.FileId = file_paths.FileId " +
    "WHERE ComputerId IN (SELECT ComputerId FROM computer_names WHERE ComputerName=?)";
  
  /**
   * Given a list of paths, it will check the files names if they exist they will be passed back
   * otherwise will be dropped.
   *
   * @param paths The paths to check
   * @return The paths that have never been present in the database.
   */
  public static List<Path> checkFilePathsExists(List<Path> paths) {
    Connection connection = ConnectionStorage.getConnection();
    try {
      PreparedStatement checkEachFileIsThere = connection.prepareStatement(checkSingleFileQuery);
      ...
      final LinkedList<Path> pathsToCheck = new LinkedList<>(paths);
      while (resultSet.next()) {
        pathsToCheck.remove(
          Paths.get(resultSet.getString("AbsoluteFilePath")));
      }
      return pathsToCheck;
    ...
    try {
      ...
      while (resultSet.next()) {
        hardDriveFiles.remove(
          new FileInformation(
            resultSet.getString("AbsoluteFilePath"),
